...
getProvider = (callback) ->
  SystemSetting.findOne({ name: 'SYSTEM_SETTING' }).exec((err, setting) ->
    ...
    switch provider
      when 'bedrock'
        callback(null, new BedrockProvider(ai.aws_region, model or 'us.anthropic.claude-sonnet-4-5-20250929-v1:0', ai.aws_role_arn or null), provider, model or 'us.anthropic.claude-sonnet-4-5-20250929-v1:0')
      when 'claude'
